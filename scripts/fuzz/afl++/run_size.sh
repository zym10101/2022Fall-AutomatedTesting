...
for idx in $(seq "$START_IDX" "$END_IDX"); do
  ...
  OUT_DIR="$SUBJECT/size_outs/out-$idx"
  if [ -d "$OUT_DIR" ]; then
    rm -rf "$OUT_DIR"
  fi
  mkdir -p "$OUT_DIR"
  ...
  # Run fuzzing
  timeout $TIMEOUT $BENCH_DIR/AFLplusplus/afl-fuzz -i "$IN_DIR" -o "$OUT_DIR" -m none -t 10000 -d -- $SUBJECT/binutils/size @@
